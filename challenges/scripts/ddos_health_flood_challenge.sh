...
# SkillRegistry per CONST-035 + CONST-050(B). Submodule cascade per
# CONST-051(A). Targets $SKILLREGISTRY_HEALTH_URL.
...
echo "=== SkillRegistry DDoS Health-Flood Challenge ==="
echo "  url=$HEALTH_URL total=$TOTAL_REQS conc=$CONCURRENCY pass≥${MIN_PASS_PCT}%"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/5] SKIP: SKILLREGISTRY_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== SkillRegistry DDoS Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== SkillRegistry DDoS Challenge: PASSED ==="
